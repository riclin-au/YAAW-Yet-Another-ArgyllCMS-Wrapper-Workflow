...
class PrinterProfilingGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Yet Another Argyll Wrapper - build v3.40.11")
    ...
    def _apply_plain_ttk_theme(self):
        ...
        try:
            ...
            # Emphasise only the main navigation and screen-edge action rows.
            # Keep exactly the platform's normal interface font size and change
            # only the weight.  Retain the named font on self: ttk styles may
            # otherwise lose a short-lived copied Font object after this method
            # returns.
 ...
 #                                  values=['CM', 'i1', '3p', 'SS', '20', '22', '41', '51'],
    ...
    def create_exec_tab(self, parent):
        ...
        ttk.Button(btn_frame, text="Step 1: Create Target", 
                   command=self.run_step1, width=20, style="YAAWAction.TButton").pack(side='left', padx=5)
        ttk.Button(btn_frame, text="Step 2: Print Target", 
                   command=self.run_step2, width=20, style="YAAWAction.TButton").pack(side='left', padx=5)
        ttk.Button(btn_frame, text="Step 3: Read Target", 
                   command=self.run_step3, width=20, style="YAAWAction.TButton").pack(side='left', padx=5)
        ttk.Button(btn_frame, text="Step 4: Build Profile", 
                   command=self.run_step4, width=25, style="YAAWAction.TButton").pack(side='left', padx=5)
