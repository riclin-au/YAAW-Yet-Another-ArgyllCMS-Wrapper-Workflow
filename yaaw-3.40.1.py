...
class PrinterProfilingGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Yet Another Argyll Wrapper - build v3.40.1")
    ...
    def _apply_plain_ttk_theme(self):
        """Use a slightly cleaner built-in ttk theme when available.

        This is intentionally conservative: no custom colours, fonts, icons,
        or external theme dependencies.  If the requested theme is unavailable,
        Tk's platform default is left untouched.
        """
        try:
            style = ttk.Style()
    ...
    def create_widgets(self):
        ...
        ttk.Button(action_frame, text="Save Config", command=self.save_config).pack(side='left', padx=(0, 5))
        ttk.Button(action_frame, text="Load Config", command=self.load_config).pack(side='left')
    ...
    def create_config_tab(self, parent):
        ...
        ttk.Label(working_frame, text="Intermediate files (ti1, ti3, etc.)",
                  foreground='gray', font=('TkDefaultFont', 8)).pack(side='left', padx=5)
        ...
        ttk.Button(targen_extra_frame, text="Help: targen", width=14,
                   command=lambda: self.open_man_page("targen")).pack(side='left', padx=5)
        ttk.Label(targen_extra_frame, text="Optional switches; see documentation",
                  foreground='gray', font=('TkDefaultFont', 8)).pack(side='left', padx=5)
 ...
 #                                  values=['CM', 'i1', '3p', 'SS', '20', '22', '41', '51'],
        ...
        ttk.Button(printtarg_extra_frame, text="Help: printtarg", width=14,
                   command=lambda: self.open_man_page("printtarg")).pack(side='left', padx=5)
        ttk.Label(printtarg_extra_frame, text="Optional switches; see documentation",
                  foreground='gray', font=('TkDefaultFont', 8)).pack(side='left', padx=5)
        ...
        # High Resolution Spectrum Mode (-H)
        hires_frame = ttk.Frame(section)
        hires_frame.pack(fill='x', pady=2)
        ttk.Label(hires_frame, text="High Resolution Spectrum:", width=25).pack(side='left')
        self.vars['chartread_highres'] = tk.BooleanVar(value=True)
        ttk.Checkbutton(hires_frame, variable=self.vars['chartread_highres']).pack(side='left', padx=5)
        ttk.Label(hires_frame, text="(-H)  Default ON for CM, OFF for i1/3p",
                  foreground='gray', font=('TkDefaultFont', 8)).pack(side='left', padx=5)

        # Suppress Warnings (-S)
        supwrn_frame = ttk.Frame(section)
        supwrn_frame.pack(fill='x', pady=2)
        ttk.Label(supwrn_frame, text="Suppress warnings:", width=25).pack(side='left')
        self.vars['chartread_supwrn'] = tk.BooleanVar(value=False)
        ttk.Checkbutton(supwrn_frame, variable=self.vars['chartread_supwrn']).pack(side='left', padx=5)
        ttk.Label(supwrn_frame, text="(-S)  Suppress wrong strip & unexpected value warnings",
                  foreground='gray', font=('TkDefaultFont', 8)).pack(side='left', padx=5)
        ...
        ttk.Button(chartread_extra_frame, text="Help: chartread", width=14,
                   command=lambda: self.open_man_page("chartread")).pack(side='left', padx=5)
        ttk.Label(chartread_extra_frame, text="Optional switches; see documentation",
                  foreground='gray', font=('TkDefaultFont', 8)).pack(side='left', padx=5)
        ...
        ttk.Button(colprof_extra_frame, text="Help: colprof", width=14,
                   command=lambda: self.open_man_page("colprof")).pack(side='left', padx=5)
        ttk.Label(colprof_extra_frame, text="Optional switches; see documentation",
                  foreground='gray', font=('TkDefaultFont', 8)).pack(side='left', padx=5)
    ...
    def _local_argyll_html(self, command):
        ...
        if self.argyll_doc_dir is not None:
            candidate = Path(self.argyll_doc_dir) / filename
    ...
    def open_man_page(self, command):
        ...
        if html_page is not None:
            try:
                opened = webbrowser.open_new_tab(html_page.as_uri())
            except Exception as exc:
                self.show_error('Could not open documentation', f'Could not open:\n\n{html_page}\n\n{exc}')
                return
            if opened is not False:
                self.log(f'Opened local Argyll documentation: {html_page}')
                return
            self.show_error('Could not open documentation', f'The web browser could not open:\n\n{html_page}')
            return
        ...
        if not self.ask_yes_no(
            ...
            'Open the official ArgyllCMS online documentation?'
        ):
            return
        ...
        try:
            ...
            self.show_error('Could not open online documentation', f'Could not open:\n\n{url}\n\n{exc}')
            return
        if opened is not False:
            self.log(f'Opened official online Argyll documentation: {url}')
        else:
            self.show_error('Could not open online documentation', f'The web browser could not open:\n\n{url}')
    ...
    def create_exec_tab(self, parent):
        ...
        ttk.Button(btn_frame, text="Step 1: Generate Target", 
                   command=self.run_step1, width=25).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="Step 2: Print Target", 
                   command=self.run_step2, width=25).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="Step 3: Read Target", 
                   command=self.run_step3, width=25).pack(side='left', padx=5)
        
        #btn_frame2 = ttk.Frame(parent)
        #btn_frame2.pack(fill='x', padx=5, pady=5)
       
        #ttk.Button(btn_frame2, text="Step 4: Build Profile",        
        ttk.Button(btn_frame, text="Step 4: Build Profile", 
                   command=self.run_step4, width=25).pack(side='left', padx=5)
        ...
        ttk.Button(control_frame, text="Show Current Settings", command=self.show_current_settings).pack(side='left', padx=5)
        ttk.Button(control_frame, text="View Gamut", command=self.show_gamut_viewer).pack(side='left', padx=5)
        ttk.Button(control_frame, text="Abort", command=self.abort_session).pack(side='right', padx=5)
...
- One-click local/online documentation buttons for targen, printtarg, chartread, and colprof
...
License: MIT
        ...
        text_widget = scrolledtext.ScrolledText(parent, wrap=tk.WORD, height=25)
    ...
    def create_field(self, parent, var_name, label, default, tooltip=""):
        ...
        if tooltip:
            ttk.Label(frame, text=f"({tooltip})", foreground='gray', 
    ...
    def show_current_settings(self):
        ...
        self.log(f"chartread: threshold={self.vars['chartread_threshold'].get()}, CIE={self.vars['chartread_cie'].get()}, high-res={self.vars['chartread_highres'].get()}, suppress warnings={self.vars['chartread_supwrn'].get()}, extra={self.vars.get('chartread_extra_args', tk.StringVar()).get()}")
    ...
    def _step3_read_target(self, basename, settings):
        """Step 3: Read printed target with ColorMunki using chartread"""
        ...
        self.log("")
        self.log("IMPORTANT: chartread will open in a separate terminal window.")
        self.log("Follow the on-screen instructions in that window.")
        ...
        # Run chartread in interactive terminal mode.  Some terminal emulators
        # (notably gnome-terminal) return control immediately after launching
        # the interactive chartread session.  Do not treat the absence of .ti3
        # at this point as an error; Step 4 performs the hard validation.
        self._run_interactive_command(cmd, cwd=working_dir)
        ...
        self.log(
            f"chartread is running for {basename}.  Complete the measurement in the terminal. "
            f"After chartread exits, continue to Step 4; YAAW will validate {ti3_file.name} then."
        )
        self.set_status("Step 3 running - complete chartread in terminal")
        return False
    ...
    def _step4_build_profile(self, basename, settings):
        ...
        # Run profcheck to validate the profile
    ...
    def _run_command(self, cmd, cwd=None):
        """Run a command while draining stdout and stderr concurrently.

        Reading only stdout before stderr can deadlock when a child fills the
        stderr pipe.  Separate reader threads preserve live logging while
        ensuring both OS pipes are continuously drained.
        """
    ...
    def _run_interactive_command(self, cmd, cwd=None):
        ...
        chartread is interactive, so it needs a real terminal attached to stdin.
        Terminal selection is centralised in open_in_terminal() so chartread,
        man-page display, and future interactive helpers behave consistently.
        """
        cmd_str = shlex.join(str(c) for c in cmd)
        self.log(f"Launching interactive command: {cmd_str}")

        title = "YAAW chartread"
        # chartread itself remains visible for the full interactive session.
        # Once the user presses D and chartread exits, let the terminal close
        # automatically rather than adding a second "Press ENTER to close" step.
        if self.open_in_terminal(title, cmd_str, cwd=cwd, hold_open=False):
            return

        self.log("WARNING: No supported terminal emulator found")
        self.show_warning(
            "No Terminal Found",
            "Could not find a supported terminal emulator.\n\n"
            "On macOS, YAAW tries Terminal.app via osascript. On Linux, it tries common terminal emulators.\n\n"
            "chartread needs to run interactively. Attempting to run anyway, but you may need to run chartread manually."
        )

        # Last-resort fallback. This may not be usable from a GUI launcher, but
        # preserves the previous behaviour for unusual environments.
    ...
    def show_gamut_viewer(self):
        ...
        # iccgamut writes <basename>.gam next to the .icc — no output path flag
        gam_path = working_dir / f"{basename}.gam"

        # --- Run iccgamut to generate the .gam hull file ---
        # -d 10 gives a reasonable surface resolution (1.0–50.0; lower = finer)
        try:
            result = subprocess.run(
                ['iccgamut', '-d', '10', str(icc_path)],
