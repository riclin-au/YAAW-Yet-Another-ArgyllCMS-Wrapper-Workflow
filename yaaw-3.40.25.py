...
Version: defined by YAAW_VERSION below
...
3.39.24: Startup now locates manual ArgyllCMS installations in common locations, adds the discovered bin directory to YAAW's PATH, and retains the matching HTML documentation directory. Help buttons prefer installed man pages, then local HTML, and ask before opening official online documentation.3.39.25: Gamut Viewer now uses a 2x2 layout with the a*b* plot and profile metrics at the top, plus separate L*a* and L*b* side views below.
3.39.26: Gamut Viewer metrics now occupy a full-size fourth panel with larger text; sRGB and AdobeRGB references use more distinct colours and dash patterns; L* slice levels are lightly marked on both side views.3.39.29: Gamut Viewer L*a* and L*b* panels are now wide, shallower side views so their hull proportions read more naturally while the top panels remain square.
...
3.40.24: Removed the redundant targen Ink Limit setting for Print RGB targets and replaced it with a fixed Device Space: Print RGB (-d2) display beside Patches.
3.40.25: The colprof Gamut Mapping (-S) field now defaults to <discovered Argyll root>/ref/sRGB.icm when that file exists; otherwise it remains blank. ICC profile browse dialogs now show both interchangeable .icc and .icm files by default.
...
# Single source of truth for the YAAW application/build version.
YAAW_VERSION = "3.40.25"


# =============================================================================
# Modular helper classes
#
# These hold logic that does not itself depend on Tk widgets, split out of
# YAAW_GUI so each piece can be read (and reasoned about) on its
# own. They are composed into YAAW_GUI in __init__ rather than
# used as mixins, so each one's dependencies are just its constructor
# arguments - not "whatever attributes happen to exist on self".
# =============================================================================

class CgatsReader:
    """Parses ArgyllCMS CGATS files (.ti1/.ti2/.ti3) and reports RGB
    endpoint diagnostics (nearest white/black, exact 100/100/100 rows, etc).

    Pure data-parsing logic extracted from YAAW_GUI: it has no Tk
    dependency beyond an injected ``log`` callback used for diagnostic
    output, so it can be read, reused, or unit-tested independently of the
    GUI.
    """
    ...
    def count_rgb_white(self, path):
        ...
        """
        try:
            rows = self.rgb_rows(path)
        except Exception as e:
            self._log(f"WARNING: Could not parse {path}: {e}")
            return 0
    ...
    def endpoint_diagnostic(self, path, stage_name, measured=False):
        ...
        try:
            ...
            self._log(f"WARNING: {stage_name}: could not parse {Path(path).name}: {e}")
        ...
        self._log(f"{stage_name}: {Path(path).name} RGB rows = {len(rows)}")
        self._log(f"{stage_name}: exact RGB 100/100/100 rows = {exact_white}; exact RGB 0/0/0 rows = {exact_black}")
        self._log(f"{stage_name}: nearest white: {self.fmt_rgb_row(nearest_white)}")
        self._log(f"{stage_name}: max RGB sum:    {self.fmt_rgb_row(max_sum)}")
        self._log(f"{stage_name}: nearest black: {self.fmt_rgb_row(nearest_black)}")
        self._log(f"{stage_name}: min RGB sum:    {self.fmt_rgb_row(min_sum)}")

        if nearest_white is None:
            self._log(f"WARNING: {Path(path).name} contains no parseable RGB rows.")
        else:
    ...
    def require_rgb_white_rows(self, path, stage_name, measured=False):
        ...
        """
        diag = self.endpoint_diagnostic(path, stage_name, measured=measured)
        return diag.get("exact_white", 0)


class CommandOutputFormatter:
    """Cleans terminal control sequences from subprocess output and
    line-buffers carriage-return progress meters into single log lines.

    Pure text processing extracted from YAAW_GUI: no Tk
    dependency beyond an injected ``log`` callback.
    """
    ...
    def log_stream_chunks(self, chunks, prefix=''):
        ...
        pending = ''
        for chunk in chunks:
            pending += self.clean_terminal_text(chunk)
            while True:
                nl = pending.find('\n')
                cr = pending.find('\r')
                ...
                if separator == '\r':
                    ...
                    self._log(f"{prefix}{piece.rstrip()}" if piece.strip() else '')
        if pending.strip():
            self._log(f"{prefix}{pending.rstrip()}")


@dataclass(frozen=True)
class WorkflowSettings:
    """Immutable snapshot of exactly the form fields a workflow step
    (targen/printtarg/chartread/colprof) is allowed to depend on.

    Tcl/Tk variables must only be read on the Tk main thread, so each
    workflow runner builds one of these (see
    YAAW_GUI._snapshot_workflow_settings) before starting its
    background worker; the worker then reads only from this object. The
    field list doubles as documentation of exactly what a step runner
    consumes - a step reading anything outside this list is a bug, and
    a typo/renamed Tk var here fails immediately at snapshot time instead
    of surfacing deep inside a running profiling step.

    ``__getitem__``/``get`` are provided so existing ``settings['x']`` /
    ``settings.get('x')`` call sites keep working unchanged.
    """
    basename: str = ""
    working_dir: str = ""
    output_dir: str = ""
    ...
    precond_profile: str = ""
    profile_desc: str = ""
    gamut_mapping_profile: str = ""
    targen_black_patches: str = ""
    targen_dark_emphasis: str = ""
    targen_extra_args: str = ""
    targen_grey_steps: str = ""
    targen_neutral_emphasis: str = ""
    targen_neutral_steps: str = ""
    targen_white_patches: str = ""
    printtarg_extra_args: str = ""
    printtarg_hexagon: str = ""
    printtarg_imgtype: str = ""
    printtarg_instrument: str = ""
    chartread_cie: str = ""
    chartread_extra_args: str = ""
    chartread_highres: str = ""
    chartread_patch_by_patch: str = ""
    chartread_resume: str = ""
    chartread_supwrn: str = ""
    chartread_threshold: str = ""
    colprof_avgdev: str = ""
    colprof_copyright: str = ""
    colprof_description: str = ""
    colprof_extra_args: str = ""
    colprof_intent: str = ""
    colprof_manufacturer: str = ""
    colprof_model: str = ""
    colprof_quality: str = ""
...
#
# This is intentionally still one file, but it's organised into sections you
# can jump to by searching for the exact banner text below (e.g. search for
# "Widget / Tab Construction"). Line numbers aren't listed here on purpose -
# they drift the moment anyone edits above a section, and a wrong number is
# worse than no number. Top-level classes:
#
#   CgatsReader              - CGATS (.ti1/.ti2/.ti3) parsing & RGB diagnostics
#   CommandOutputFormatter   - terminal-output cleanup / progress-line collapsing
#   WorkflowSettings         - typed snapshot of the fields a workflow step reads
#   _ArgyllHtmlParser        - minimal stdlib-only HTML renderer for Argyll docs
#   YAAW_GUI      - the application; sections inside it, in order:
#
#       "Startup: Theme, ArgyllCMS Detection, Window Sizing"
#       "Widget / Tab Construction"
#       "ArgyllCMS Documentation Viewer (man pages / native-Tk HTML render)"
#       "Dialog Box Helpers (YAAW's own compact message/confirm dialogs)"
#       "Logging (screen log + persistent per-project log file)"
#       "Config / Session Persistence (project JSON + crash-recovery session)"
#       "CGATS validation helpers"            (thin delegators to CgatsReader)
#       "Preconditioning safety helpers"
#       "Workflow Step Runners"               (run_step1..4)
#       "Workflow Step Implementations"       (targen/printtarg/chartread/colprof)
#       "Command Execution Helpers"           (subprocess/pty plumbing)
#       "Gamut Viewer (iccgamut + X3DOM, resizable 2x2 Tk canvas plots)"
# =============================================================================
...
# ICC profiles, gamut mapping profiles, or preconditioning profiles.
#
...
class YAAW_GUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(f"Yet Another Argyll Wrapper (or Workflow) - build v{YAAW_VERSION}")
        ...
        # Composed helpers: pure logic with no Tk dependency beyond the
        # self.log callback. See the "Modular helper classes" block near
        # the top of this file.
        self._cgats = CgatsReader(self.log)
        self._cmd_output = CommandOutputFormatter(self.log)
    ...
    def _default_gamut_mapping_profile(self):
        """Return Argyll's bundled sRGB reference profile when available.

        Manual Argyll distributions keep reference profiles beside ``bin`` and
        ``doc`` under the installation root.  Repository layouts may not expose
        that tree, so absence of the file deliberately produces a blank default.
        """
        if self.argyll_install_root is None:
            return ""
        profile = self.argyll_install_root / 'ref' / 'sRGB.icm'
    ...
    def create_config_tab(self, parent):
        ...
        # Row 0: Device Space, Patches, Grey Steps, Neutral Steps
        grid_label(0, 0, "Device Space:")
        device_space_entry = tk.Entry(
            grid_frame,
            width=8,
            relief='sunken',
            borderwidth=1,
        ...
        device_space_entry.insert(0, "Print RGB")
        ...
        self.patches_combo.grid(row=0, column=4, sticky='w', padx=(0, 2), pady=3)
 ...
 #                                  values=['CM', 'i1', '3p', 'SS', '20', '22', '41', '51'],
        ...
        # Gamut Mapping profile selection
        ...
        ttk.Label(source_frame, text="Gamut Mapping:", width=25).pack(side='left')
        self.vars['gamut_mapping_profile'] = tk.StringVar(value=self._default_gamut_mapping_profile())
        source_entry = ttk.Entry(source_frame, textvariable=self.vars['gamut_mapping_profile'], width=60)
        source_entry.pack(side='left', padx=5)
        self.vars['gamut_mapping_profile'].trace_add('write', lambda *a: source_entry.after(10, lambda: source_entry.xview_moveto(1)))
        browse_source_btn = ttk.Button(source_frame, text="Browse...", command=self.browse_gamut_mapping_profile)
    ...
    def browse_gamut_mapping_profile(self):
        """Browse for rendering ICC profile"""
        current_file = self.vars['gamut_mapping_profile'].get()
        ...
        if filename:
            self.vars['gamut_mapping_profile'].set(filename)
...
using ArgyllCMS and a suitable spectrophotometer (e.g. - Colormunki, i1Pro, or i1Pro3+)
...
5. Build and validate ICC profile (colprof and profcheck).
...
- One-click documentation buttons for targen, printtarg, chartread, and colprof, displayed in a built-in viewer
...
Although YAAW will function fully without other external tools, an HTML5 browser is required for X3DOM 3D visualizations
...
If not installing ArgyllCMS from the system repositories, it should be installed at /opt or /usr/local and permissions confirmed.

For more information:
- ArgyllCMS: https://www.argyllcms.com/

Version: {YAAW_VERSION}
Copyright: Richard Lindner and contributors
Coding assistance: Anthropic Claude; OpenAI ChatGPT
License: MIT
        """
    ...
    def show_current_settings(self):
        ...
        self.log(f"Target: Print RGB (-d2), {patches} patches, instrument {self.vars['printtarg_instrument'].get()}, printtarg paper {paper_display}")
        ...
        if self.vars['gamut_mapping_profile'].get():
            self.log(f"Rendering/source profile: {self.vars['gamut_mapping_profile'].get()}")
    ...
    def _current_config_payload(self):
        ...
        return {
            ...
            'yaaw_config_version': YAAW_VERSION
        }
    ...
    def abort_session(self):
        ...
        defaults = {
            ...
            'gamut_mapping_profile': self._default_gamut_mapping_profile(),
    ...
    # --- CGATS parsing/diagnostics: delegated to CgatsReader (self._cgats). ---
    # These thin wrappers exist purely so every existing call site in this
    # class (self._read_cgats_sections(...), self.count_rgb_white_rows(...),
    # etc.) keeps working unchanged; the actual logic lives in the
    # module-level CgatsReader class above, which has no Tk dependency.

    def _read_cgats_sections(self, path):
        return self._cgats.read_sections(path)

    def _rgb_rows_from_cgats(self, path):
        return self._cgats.rgb_rows(path)

    def count_rgb_white_rows(self, path):
        return self._cgats.count_rgb_white(path)

    def _fmt_rgb_row(self, row):
        return self._cgats.fmt_rgb_row(row)

    def rgb_endpoint_diagnostic(self, path, stage_name, measured=False):
        return self._cgats.endpoint_diagnostic(path, stage_name, measured=measured)

    def max_lab_l_for_rgb_white(self, path):
        return self._cgats.max_lab_l_for_rgb_white(path)

    def require_rgb_white_rows(self, path, stage_name, measured=False):
        return self._cgats.require_rgb_white_rows(path, stage_name, measured=measured)
    ...
    def _snapshot_workflow_settings(self):
        """Return a WorkflowSettings snapshot for worker threads.
        ...
        resulting immutable WorkflowSettings for command construction and paths.

        Built from dataclasses.fields(WorkflowSettings) rather than a hand-typed
        field list, so the dataclass definition is the single source of truth;
        a missing/renamed Tk var raises a KeyError here immediately, rather than
        surfacing as a wrong value deep inside a running profiling step.
        """
        return WorkflowSettings(**{
            f.name: self.vars[f.name].get() for f in fields(WorkflowSettings)
        })
    ...
    def _step4_build_profile(self, basename, settings):
        ...
        # Add gamut mapping profile if specified
        gamut_mapping_profile = settings['gamut_mapping_profile']
        if gamut_mapping_profile:
            cmd.extend(['-S', gamut_mapping_profile])
    ...
    # --- Terminal-output cleanup: delegated to CommandOutputFormatter
    # (self._cmd_output). Thin wrappers so every existing call site keeps
    # working unchanged (including the several call sites in the chartread
    # pty handling further below, which stay as self._clean_terminal_text(...)).

    def _clean_terminal_text(self, text):
        return self._cmd_output.clean_terminal_text(text)

    def _log_stream_chunks(self, chunks, prefix=''):
        return self._cmd_output.log_stream_chunks(chunks, prefix=prefix)
