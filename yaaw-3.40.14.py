...
class PrinterProfilingGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Yet Another Argyll Wrapper - build v3.40.14")
 ...
 #                                  values=['CM', 'i1', '3p', 'SS', '20', '22', '41', '51'],
    ...
    def create_exec_tab(self, parent):
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, height=20, wrap=tk.WORD, font=('TkFixedFont', 10), background=self._yaaw_output_bg)
...
License: MIT
        ...
        text_widget = scrolledtext.ScrolledText(parent, wrap=tk.WORD, height=25, background=self._yaaw_output_bg)
    ...
    def _step4_build_profile(self, basename, settings):
        ...
        self._run_command_head_to_project_log(
            ...
            cwd=working_dir,
            max_lines=10,
            heading="profcheck highest delta-E results (first 10 output lines):"
        )
    ...
    def _run_command_head_to_project_log(self, cmd, cwd=None, max_lines=10, heading=None):
        """Run a command and write only its first ``max_lines`` to the project log.

        stdout and stderr are combined to match YAAW's normal command handling.
        The complete output is consumed in Python before selecting the leading
        lines, avoiding a shell pipeline and any dependency on ``head``.
        """
        ...
        lines = cleaned.splitlines()[:max(0, int(max_lines))]
